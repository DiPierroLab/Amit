...
class ChromoChip8(object):
    ...
    def __init__(self, traj, snap_index, energy_bin_width, energy_sigma, chipfile, chromatin_bead_size):
        ...
        energy_sigma = mean_be*energy_sigma
        self.norm_be = np.random.normal(mean_be,energy_sigma,n_be)
        print(mean_be,n_be,energy_sigma,np.mean(self.norm_be))
        
        input_bins = np.arange(0, self.norm_be.max()+energy_bin_width, energy_bin_width)
        count, bins = np.histogram(self.norm_be, bins = input_bins)           
        ...
        for pos in pattern_sphere:
            ...
            CBS_particle.name = 'X'+ str( find_nearest(self.coarse_chip_seq, self.norm_be[jj]) )
