# This file is an edited version of the file that simulates the fully random distrbution of binding sites
# Here the volume of the chromosome is calculated using a convex-hull and then arranging the binding sites
# in a random sphere of the same volume
...
def points_on_unit_sphere(npoints, ndim=3):
    vec = np.random.randn(ndim, npoints)
    vec /= np.linalg.norm(vec, axis=0)
    return vec

def points_in_sphere(npoints,radius):
    pp = points_on_unit_sphere(npoints)
    rn = np.random.random([npoints,1])
    rn = rn**(1.0/3.0)*radius
    pp = pp.T
    pp1 = pp*rn
    return pp1
...
class ChromoChip9(object):
    ...
    def __init__(self, traj, snap_index, loaded, energy_bin_width, chipfile, chromatin_bead_size, generate_sites):
        ...
        input_bins = np.arange(0, self.all_subsites_en.max()+energy_bin_width, energy_bin_width)
        count, bins = np.histogram(self.all_subsites_en.flat, bins = input_bins)           
        ...
        if generate_sites:
            ...
            # use convex-hull to calcualte the volume
            hull = ConvexHull(self.chromosome.xyz)
            rad = (3*hull.volume/(4.0*np.pi))**(1.0/3.0)
            #dim = np.mean(xl)
            #rad = dim*0.5
            print(rad)
            pattern_sphere = points_in_sphere(n_be, rad)
            hull2 = ConvexHull(pattern_sphere)
            new_rad = rad*(hull2.volume/hull.volume)**(1.0/3.0)
            pattern_sphere = points_in_sphere(n_be, new_rad)
            
            jj = 0
            for pos in pattern_sphere:
                CBS_particle = mb.clone(CBS_proto)
                CBS_particle.name = 'X'+ str( find_nearest(self.coarse_chip_seq, self.all_subsites_en[jj]) )
                CBS_particle.translate(pos)                    
                self.chr_binding_sites.add(CBS_particle)
                jj += 1
...
#         print(snapshot.box.Ly,yy.max(0)-yy.min(0))
    ...
    def simulate_pair_sigmoid(self, gam=0.01, kT=1, dt = 0.01, teq=1e5, tsim=2e5, outfile='sim_out', freqout=1000, seed =12345):
