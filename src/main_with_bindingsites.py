...
#         print(snapshot.box.Ly,yy.max(0)-yy.min(0))
    ...
    def simulate_pair_sigmoid_full(self, gam=0.01, kT=1, dt = 0.01, teq=1e5, tsim=1e6, outfile='sim_out', freqout=1000, restart_freq=10000, seed =12373):
        ...
        #Logging sim data
        hoomd.analyze.log(filename = outfile + '.log', quantities= ['pair_lj_energy','pair_table_energy','potential_energy','kinetic_energy','temperature'], period=freqout, header_prefix='#', phase=0, overwrite=True)

        #Run MD for equilibration, before dumping
        hoomd.run(teq)
        
        # Dump gsd traj
        hoomd.dump.gsd(outfile + '.gsd',  period=freqout, group=tf_group, phase=0, overwrite=True)         
        gsd_restart = hoomd.dump.gsd(filename=outfile + '_restart.gsd', group=tf_group, truncate=True, period=restart_freq, phase=0)
