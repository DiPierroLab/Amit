...
def get_radgy(xx):
    import numpy as np
    r_com = np.mean(xx,axis=0)
    dx = xx - r_com
    Rg = np.sqrt(np.mean(np.sum(dx**2,axis=1),axis=0))
    return(Rg)
...
class ChromoChip2(object):
    ...
    def __init__(self, traj, snap_index, loaded, energy_bin_width, chipfile, chromatin_bead_size, generate_sites, base_resolution, ref_Rg):
        ...
        # read Chromosome configuration: read trajectory and pass it to mbuild for adding TF particles                
            newtraj = loaded
        ...
        # analyze and process ChIP-seq genome track

        ref_genome_pos = chip_seq[:,0]
        n_ref_chr = len(ref_genome_pos)
        i_whole = np.mod(ref_genome_pos,10) != 0
        ref_genome_pos[i_whole] -= 1
        ...
        all_be_subsites = []  # collecting all binding energies of all the sub-sites
        for i in range(n_ref_chr):
            xa = all_be[i,:]
            ii = np.where(xa > 0)
            nn = len(ii[0])
            if nn > 0:
                all_be_subsites.extend(xa[ii])
            else:
                all_be_subsites.append(0.0)
                
        self.all_subsites_en = np.array(all_be_subsites)
        mean_be = np.mean(all_be_subsites)
        n_be = len(all_be_subsites)
        ...
        if generate_sites:
            # determine which indices to use from ChIP-seq genome track
            n_real_chr = self.chromosome.n_particles
            bead_edges = np.arange(0,n_real_chr*base_resolution+base_resolution,base_resolution)
            bead_centers = (bead_edges[1:]+bead_edges[:-1])/2.0

            y_equal, i_real, j_ref = np.intersect1d(bead_centers,ref_genome_pos,return_indices=True)

            # scaling the configuration according to ref genome size

            real_Rg = get_radgy(self.chromosome.xyz)            
            scale_factor = ref_Rg/real_Rg
            print(real_Rg, ref_Rg, scale_factor)
            self.chromosome.xyz *= scale_factor 
            ...
            ll=0
            for i in range(n_ref_chr):
                cpos = self.chromosome.xyz[i_real[i],:]
                xa = all_be[i,:]
                ii = np.where(xa > 0)
                nn = len(ii[0])
...
#         chr_binding_sites.translate_to(origin)  
          ...
          #real_Rg = get_radgy(chr_binding_sites.xyz) 
          #scale_factor = ref_Rg/real_Rg
          #chr_binding_sites.xyz = chr_binding_sites.xyz*scale_factor
...
#         print(snapshot.box.Ly,yy.max(0)-yy.min(0))
    ...
    def sim_param_pair_sigmoid_full(self, sigmaX=1, sigmaTF=1, epsTF = 1, rcut_tf = 2.5, rcut_chtf = 2.5, ch_tf_range = 3, s = 1, inpfile = 'input_no_top.gsd', restartfile='restart_no_top.gsd'):
        '''Provide simulation parameters'''
...
#         print(snapshot.box.Ly,yy.max(0)-yy.min(0))
    ...
    def simulate_pair_sigmoid_full(self, gam=0.01, kT=1, dt = 0.01, teq=1e5, tsim=1e6, outfile='sim_out', freqout=1000, restart_freq=10000, seed =12373):
...
#         hoomd.run_upto(teq+tsim) 
        ...
        try:
            hoomd.run_upto(tsim, limit_multiple=10000)
        except WalltimeLimitReached:
            pass

        gsd_restart.write_restart()    

    def simulate_pair_sigmoid_restart(self, gam=0.01, kT=1, dt = 0.01, teq=1e5, tsim=1e6, outfile='sim_out', freqout=1000, restart_freq=10000, seed =12373):
