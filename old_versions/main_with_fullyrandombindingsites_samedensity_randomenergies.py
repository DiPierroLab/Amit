...
class ChromoChip7(object):
    ...
    def __init__(self, traj, snap_index, energy_bin_width, chipfile, chromatin_bead_size):
        ...
        for i in range(self.chromosome.n_particles):
            ...
            if nn > 1:
                ll += 1    
                pattern_sphere = points_in_sphere(nn, rad)
#                 rn = np.random.random([3,nn])*rad
#                 pattern_sphere *= rn
#                 pattern_sphere = pattern_sphere.T
#                 print(cpos)
                m = 0
                for pos in pattern_sphere:
                    CBS_particle = mb.clone(CBS_proto)
#                     CBS_particle.name = 'X'+ str( find_nearest(self.coarse_chip_seq, xa[m]) )
                    be_rand = np.random.rand()*all_be.max()
                    CBS_particle.name = 'X'+ str( find_nearest(self.coarse_chip_seq, be_rand) )
...
#                 chr_bead_sites.translate(cpos)
                ...
                pattern_sphere = points_in_sphere(1, rad)
#                 rn = np.random.random([3,1])*rad
#                 pattern_sphere *= rn
#                 pattern_sphere = pattern_sphere.T
#                 print(pattern_sphere)
                CBS_particle = mb.clone(CBS_proto)
#                 CBS_particle.name = 'X'+ str( find_nearest(self.coarse_chip_seq, xa[0]) )
                be_rand = np.random.rand()*all_be.max()
                CBS_particle.name = 'X'+ str( find_nearest(self.coarse_chip_seq, be_rand) )
                CBS_particle.translate(pattern_sphere[0,:])        
                chr_bead_sites.add(CBS_particle)
