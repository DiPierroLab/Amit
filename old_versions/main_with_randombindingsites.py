...
class ChromoChip3(object):

    '''A simple polymer class to load chromosome structure and assign bead types based on Chip-seq dictionary
    INPUT: traj and topology input for mdtraj (gro and dcd/h5) AND snapshot index AND chip_seq affinity list of energy multiples of kT, e.g [1.2, 1.4, 1.5, 100, ...]. This particular class also organizes the binding sites randomly in space.
        '''

    def __init__(self, traj, snap_index, energy_bin_width, chipfile, chromatin_bead_size, box_length):
